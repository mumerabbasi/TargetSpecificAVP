...
def _sequence_run_name(
        town: str,
        pose_source: str,
        seed: int,
        attempt_idx: int) -> str:
    return "{}_{}_seed{:03d}_try{:02d}".format(
        town, pose_source, seed, attempt_idx + 1)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a fresh multi-sequence pursuit suite.")
    parser.add_argument(
        "--towns",
        nargs="+",
        default=[
...
def _sequence_summary(
        summary: Dict[str, object], summary_path: str) -> Dict[str, object]:
    ...
    return {
        ...
        "pose_metrics": {
            ...
            "dx_abs_mean_m": float(
                pose_metrics["all_pose_frames"]["dx_abs_m"]["mean"]
            ),
            "dy_abs_mean_m": float(
                pose_metrics["all_pose_frames"]["dy_abs_m"]["mean"]
            ),
            "yaw_abs_mean_deg": float(
                pose_metrics["all_pose_frames"]["yaw_abs_deg"]["mean"]
            ),
            ...
            "latency_mean_ms": float(
                pose_metrics["all_pose_frames"]["latency_ms"]["mean"]
            ),
            "mask_iou_mean": float(
                pose_metrics["all_pose_frames"]["mask_iou"]["mean"]
            ),
        },
        "pursuit_quality": {
            "distance_error_abs_mean_m": float(
                pursuit_quality["distance_error_abs_m"]["mean"]
            ),
            "lateral_error_abs_mean_m": float(
                pursuit_quality["lateral_error_abs_m"]["mean"]
            ),
            "follow_yaw_abs_mean_deg": float(
                pursuit_quality["follow_yaw_abs_deg"]["mean"]
            ),
            "within_follow_band_ratio": float(
                pursuit_quality["within_follow_band_ratio"]
            ),
    ...
    for town in args.towns:
        for pose_source in args.pose_sources:
            ...
            for attempt_idx in range(int(args.attempts_per_sequence)):
                ...
                run_name = _sequence_run_name(
                    town, pose_source, seed, attempt_idx)
                config = PursuitEvalConfig(
                    pose_source=pose_source, town=town,
                    carla_host=args.carla_host, carla_port=int(
                        args.carla_port),
                    random_seed=seed, num_frames=int(
                        args.frames_per_sequence),
                    output_dir=args.output_dir, run_name=run_name,
                    num_background_vehicles=int(
                        args.num_background_vehicles),
                    initial_target_distance_m=float(
                        args.initial_target_distance),
                    ego_initial_speed_mps=float(args.ego_initial_speed),
                    target_speed_difference_pct=float(
                        args.target_speed_difference),
                    stop_on_follow_guard_breach=False,
                    target_out_of_view_breach_frames=20,
                    ego_offroad_breach_frames=15, sam3_device=args.sam3_device,
            ...
            if last_summary is None or last_summary_path is None:
                raise RuntimeError(
                    "No pursuit run completed for {} / {}.".format(
                        town,
                        pose_source,
                    )
                )
